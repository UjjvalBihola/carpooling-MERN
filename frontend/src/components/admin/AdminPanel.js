    ...
    const fetchTripsAndUsers = async () => {
      try {
        ...
        for (let trip of tripsData) {
          const originAddress = await Geocode.fromLatLng(
            trip.source.lat,
            trip.source.lng
          );
          const destinationAddress = await Geocode.fromLatLng(
            ...
            <tbody>
              {trips.map((trip) => (
                <tr key={trip._id}>
                  ...
                  <td>
                    <a
                      href={`https://maps.google.com/?q=${trip.source.lat},${trip.source.lng}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {trip.sourceAddress ||
                        `Lat: ${trip.source.lat}, Lng: ${trip.source.lng}`}
                    </a>
                  </td>
                  <td>
                    <a
                      href={`https://maps.google.com/?q=${trip.destination.lat},${trip.destination.lng}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {trip.destinationAddress ||
          ...
          <div className="modal-content">
            ...
            {selectedTrip && (
              <>
                <h2>Trip Details</h2>
                <p>
                  <strong>ID:</strong> {selectedTrip._id}
                </p>
                <p>
                  <strong>Origin:</strong>{" "}
                  <a
                    href={`https://maps.google.com/?q=${selectedTrip.source.lat},${selectedTrip.source.lng}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    {selectedTrip.sourceAddress}
                  </a>
                </p>
                <p>
                  <strong>Destination:</strong>{" "}
                  <a
                    href={`https://maps.google.com/?q=${selectedTrip.destination.lat},${selectedTrip.destination.lng}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                ...
                <p>
                  <strong>Date:</strong>{" "}
                  {new Date(selectedTrip.dateTime).toLocaleString()}
                </p>
                <p>
                  <strong>Status:</strong>{" "}
                  {selectedTrip.completed ? "Completed" : "In Progress"}
                </p>
              </>
            )}
            {selectedUser && (
